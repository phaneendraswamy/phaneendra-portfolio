  ...
  return (
    <div className="fixed bottom-5 right-5 z-40 hidden xl:block">
      <button
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full border border-ocean/30 bg-slate-950/90 text-ocean shadow-[0_18px_50px_rgba(14,165,233,0.22)] backdrop-blur transition hover:scale-[1.03] hover:border-ocean/50 dark:border-white/10"
      ...
      <aside
        className={`absolute bottom-16 right-0 w-[19rem] max-h-[calc(100vh-8rem)] overflow-y-auto rounded-3xl border border-slate-200 bg-white/95 p-3 shadow-glow backdrop-blur transition-all duration-300 dark:border-white/10 dark:bg-slate-950/95 2xl:w-[22rem] 2xl:p-4 ${
          isOpen ? 'pointer-events-auto translate-y-0 opacity-100' : 'pointer-events-none translate-y-4 opacity-0'
        }`}
      >
        <div className="flex items-start gap-3">
          <div className="rounded-2xl bg-ocean/10 p-2.5 text-ocean">
        ...
        <div className="mt-4 space-y-2">
          {prompts.map((item) => (
            <button
              ...
              className={`w-full rounded-2xl border px-3 py-2.5 text-left text-sm transition duration-300 ${
        ...
        {selectedPrompt && (
          <div className="mt-4 rounded-2xl border border-ocean/20 bg-ocean/10 px-3 py-3 transition-all duration-300">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-ocean">AI response</p>
            <div className="mt-3 space-y-3">
              <div className="max-h-72 overflow-y-auto rounded-2xl border border-white/30 bg-white/60 px-3 py-3 dark:border-white/10 dark:bg-slate-950/30">
        ...
        <div className="mt-4 flex items-center gap-2 rounded-2xl border border-mint/20 bg-mint/10 px-3 py-3 text-sm text-mint">
          <Sparkles size={16} />
